...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-ocean-200">
      <div className="container-luxury">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            />
            <span className="font-serif text-2xl tracking-[0.1em] text-ocean-950">
              KABSAT
            </span>
          </Link>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-sm uppercase tracking-[0.15em] transition-colors ${
                  isActive(link.href)
                    ? "text-ocean-950 font-medium"
                    : "text-ocean-600 hover:text-ocean-950"
                }`}
              >
                {link.label}
              </Link>
